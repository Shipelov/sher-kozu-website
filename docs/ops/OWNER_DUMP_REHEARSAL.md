...
| Dump | `mysqldump --single-transaction --skip-lock-tables --set-gtid-purged=OFF --hex-blob --default-character-set=utf8mb4 --column-statistics=0 --no-tablespaces` всей базы. Если mysqldump падает и `dump_tool=auto`, скачивается TiDB toolkit с `download.pingcap.org` с проверкой sha256 и дамп снимает Dumpling (`--filetype sql`, файлы схемы и данных склеиваются в один) | только чтение |
...
| Import | `mysql < owner-dump.sql` | база `test` |
...
| `dump_tool` | `auto` | `mysqldump` без fallback, `dumpling` сразу |
| `dumpling_version` | `v8.5.1` | версия toolkit для fallback |
