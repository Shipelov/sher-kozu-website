import { ArrowDown, CalendarRange, CheckSquare, Crown, MessageCircle, Pin, Square, Trash2, Users } from "lucide-react";
...
type PostsListRow = {
  ...
  extraActions?: { label: string; icon?: React.ReactNode; href: string }[];
    ...
    <>
      <EntityFormCard title={eventForm.id ? "Редактировать событие" : "Новое событие"} description="Используйте короткие формулировки, чтобы карточки легко читались в Club Feed." footer={<div className="flex flex-wrap gap-3"><Button onClick={onSubmit} disabled={submitDisabled}>{eventForm.id ? "Сохранить событие" : "Создать событие"}</Button><Button variant="outline" onClick={onResetForm}>Очистить</Button></div>}>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Лимит мест" error={eventErrors.maxCapacity}><Input type="number" min={0} placeholder="0 = без ограничений" aria-invalid={Boolean(eventErrors.maxCapacity)} value={eventForm.maxCapacity} onChange={(e) => { const val = Math.max(0, Number(e.target.value) || 0); onEventFormChange({ ...eventForm, maxCapacity: val }); if (eventErrors.maxCapacity) onEventFieldErrorClear("maxCapacity"); }} /></Field>
        </div>
        ...
        <div className="flex items-center justify-between rounded-xl border border-green-100 bg-green-50/30 px-4 py-3"><div><p className="font-medium text-stone-950">Регистрация открыта</p><p className="text-sm text-stone-500">Когда выключено, новые записи не принимаются.</p></div><Switch checked={eventForm.registrationOpen} onCheckedChange={(checked) => onEventFormChange({ ...eventForm, registrationOpen: checked })} /></div>
...
export function buildAdminClubEventsTabProps(props: AdminClubEventsTabContainerProps): AdminClubEventsTabContentProps {
  return {
    ...
    onResetForm: () => { props.setEventForm({ id: undefined, title: "", dateLabel: "", description: "", status: "", tone: "", sortOrder: 0, hidden: false, maxCapacity: 0, registrationOpen: true }); props.setEventErrors({}); },
