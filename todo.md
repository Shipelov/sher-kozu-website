...
## LLM-генерация пресетов + возраст животного (06.04.2026)
- [x] Добавить поле birthDate в схему animals и пушнуть миграцию (поле уже существует)
- [x] Создать серверную tRPC-процедуру для LLM-генерации уникальных данных животного
- [x] Обновить AdminAnimals: заменить статические пресеты на LLM-вызов с loading-индикатором
- [x] Добавить DatePicker для даты рождения в форму создания/редактирования животного (type=date)
- [x] Показать возраст (x лет, y месяцев) в профиле животного рядом с кличкой
- [x] Показать обратный отсчёт дней до дня рождения в профиле животного
- [x] Написать тесты для LLM-генерации и расчёта возраста (19 тестов)

## Доработка LLM-генерации: ручной выбор вида и породы (06.04.2026)
- [x] Определить список пород: козы (Англо-нубийская, Альпийская, Зааненская, Тоггенбургская), овцы (Остфризская, Лакон, Романовская, Ассаф)
- [x] Обновить серверную процедуру generatePreset: принимать species+breed, генерировать остальное
- [x] Заменить две кнопки на выпадающие списки Вид → Порода → кнопка «Сгенерировать»
- [x] Протестировать обновлённый flow (21 тест, все проходят)

## Исправление пород и процесса создания животного (06.04.2026)
- [x] Оставить только 2 породы: козы (Англо-нубийская, Альпийская), овцы (Лакон, Остфризская)
- [x] Убрать лишние породы (Зааненская, Тоггенбургская, Романовская, Ассаф) из UI и тестов (faqChat оставлен как справочная база знаний)
- [x] Исправить баг: улучшена валидация — показывает конкретно, какие поля не заполнены (имя, slug, вид, порода, описание)
- [x] Исправить баг: породы теперь корректно показываются для обоих видов (козы и овцы)
- [x] Порода стала выпадающим списком (зависит от вида), сбрасывается при смене вида

## Баг: зависание браузера при создании животного (06.04.2026)
- [x] Исправить зависание браузера при нажатии «Создать карточку» после выбора вида и породы (диагностика: тост валидации работал, но был малозаметен; z-index Toaster повышен до 99999)

## Горизонт 1 — Техническая гигиена (по результатам аудита, апрель 2026)

- [x] Очистить репозиторий от 24 временных файлов (.tmp_*, .adminclub_*)
- [x] Удалить .manus-logs и .manus-notes из git tracking
- [x] Обновить .gitignore — добавить паттерны для временных файлов
- [x] Аудит секретов — проверить отсутствие API-ключей и паролей в коде (чисто: все секреты через process.env, plainPassword удалён)
- [x] Создать README.md с описанием проекта, стека и инструкцией по запуску
- [x] Финальный smoke-тест всех основных маршрутов (12/12 OK, 0 критических ошибок)

## Горизонт 1.5 — Контент и стабильность (апрель 2026)

- [x] Исследовать ошибку trackVisit — транзиентные сбои TiDB, самоустранились, 100% success rate сейчас
- [x] Наполнить клубную ленту: 5 записей дневника + 2 события + 3 участника; очищено 165 тестовых постов и 166 тестовых событий

## Переработка админ-панели клуба (апрель 2026)

- [x] Добавить поле hidden в схему БД (clubPosts, clubEvents, clubMembers)
- [x] Обновить серверные роутеры — bulk hide/unhide/delete мутации
- [x] Обновить публичную ленту — фильтровать скрытые записи
- [x] Переработать UI постов: таблица + массовые действия (скрыть/показать/удалить) + фильтр видимости + бейдж «Скрыт» + переключатель hidden в форме
- [x] Переработать UI событий: аналогично постам
- [x] Переработать UI участников: аналогично постам
- [ ] Inline-редактирование в таблице (заголовок, порядок, статус)
- [x] Проверить в браузере и запустить тесты
- [x] Исправить наложения кнопок на тексты в блоках «Текущие посты» и «События клуба»

## Club Interactivity — Likes, Comments, Event Registration

- [x] Создать таблицы БД для лайков постов, комментариев к постам и регистраций на события
- [x] Реализовать серверные tRPC-процедуры для лайков (toggle), комментариев (CRUD) и регистрации на события
- [x] Добавить интерактивные лайки (сердечки) на посты в публичной ленте клуба
- [x] Добавить возможность оставлять и просматривать комментарии к постам клуба
- [x] Реализовать кнопку «Записаться» на события с учётом политики статусов (владелец/гость/неавторизованный)
- [x] Добавить admin-инструменты для управления лайками, комментариями и регистрациями на события
- [x] Написать тесты для лайков, комментариев и регистрации на события

## Push-уведомления, модерация комментариев, лимит мест

- [x] Push-уведомления владельцу при новом комментарии к посту
- [x] Push-уведомления владельцу при новой регистрации на событие
- [x] Автоматическая модерация комментариев (фильтрация нежелательного контента)
- [x] Добавить поле maxCapacity в форму создания/редактирования события в админке
- [x] Автоматически закрывать регистрацию при заполнении лимита мест
- [x] Написать тесты для уведомлений, модерации и лимита мест

- [x] Исправить ошибки дублирующихся React key в списках постов и событий админ-панели клуба
- [x] Исправить слишком строгую модерацию комментариев — блокирует нормальные сообщения
- [x] Toast-уведомление при блокировке комментария модерацией вместо необработанной ошибки
- [x] Усилить модерацию комментариев — фильтр пропускает нецензурную речь
- [x] Исправить push-уведомления о комментариях — проверено, сервер отправляет успешно
- [x] Расширить фильтр модерации: жопа, срать, дрочить, залупа, минет, член/хер/хрен в контексте, нахер, лох, даун, дебил, урод и др.
- [x] Исправить кнопку «Поделиться» — при нажатии ничего не происходит

## Contact Information

- [x] Добавить email info@sherkozu.ru в футер сайта (заметно, на всех страницах)
- [x] Добавить email info@sherkozu.ru в навигационную панель или хедер
...
- [x] BUG: Mobile loading still freezes ~30s after Gate login — fixed: VDS had stale build (wrong IP 89.169.131.47 vs real 89.111.165.77). Deployed via GitHub Actions + lazy FarmMap/Yandex Maps via IntersectionObserver + removed broken analytics script + conditional Telegram SDK
...
- [x] Add Excel/PDF report export to АРМ Дояр (milking sessions with all fields, period selector)
- [x] Add Excel/PDF report export to АРМ Сыродел (receptions + tank turnover, period selector)
- [x] Add Excel/PDF report download to Admin Overview (summary stats, already has period selector)
...
- [x] Post-publish: Verify Masha and Zoya answer through the Cloudflare route on koza.vip
...
- [x] Post-Worker-deploy: Verify system.aiDiagnostics returns HTTP 200 instead of unsupported_country_region_territory
- [x] Post-Worker-deploy: Verify real Masha tRPC and Zoya SSE responses on koza.vip
...
- [x] Post-model-switch: Verify Masha and Zoya real responses on koza.vip
...
- [x] Zoya sports menu incident: Воспроизвести ошибку длинного запроса и установить точную причину в общем tRPC/SSE-контуре
- [x] Zoya sports menu grounding: Использовать только реальные доступные владельцу продукты и не выдумывать сорта, КБЖУ или наличие
- [x] Zoya sports menu safety: Не строить дневной рацион только из сыров и учитывать полный баланс питания, гидратацию и ограничения насыщенных жиров/натрия
- [x] Zoya sports menu intent: Корректно интерпретировать «20% своей молочной продукции» и запрашивать уточнение при неоднозначности
- [x] Zoya sports menu calculations: Рассчитывать персональный диапазон белка и распределение по приёмам пищи без медицинской диагностики
...
- [x] Zoya menu UX audit: Определить доступные в профиле возраст/рост/вес/активность и реальные nutrition-поля confirmed-продуктов
- [x] Zoya menu dialogue: Заменить длинный противоречивый шаблон одним кратким уточняющим вопросом при недостатке данных
- [x] Zoya menu dialogue: Не показывать «силовые тренировки», если источник данных неясен или текущий запрос начинает новый расчёт
- [x] Zoya menu calculation: После уточнения смысла 30% рассчитывать граммовки и итоговые КБЖУ с явным разделением фактов и оценок
- [x] Zoya menu calculation: Использовать только confirmed-сыры и не выдавать справочные КБЖУ за лабораторный состав продукта фермы
- [x] Zoya menu shared flow: Обеспечить одинаковый многошаговый сценарий для tRPC и SSE без обращения к LLM
- [x] Zoya menu UX tests: Покрыть исходный запрос пользователя, уточнение 30% и финальный расчёт без повторов/противоречий
- [x] Zoya menu UX validation: Выполнить полный Vitest, TypeScript и production build перед чекпоинтом
- [x] Zoya menu state incident: Воспроизвести точный диалог «меню на завтра → 30% по массе → 2500 кал → сбалансированное питание»
- [x] Zoya menu state: Распознавать короткие ответы «2500 кал/ккал» как продолжение активного расчёта
- [x] Zoya menu state: Не завершать mass-share расчёт без общей массы рациона и не передавать неполный сценарий в LLM
- [x] Zoya menu state: Поддерживать корректирующие продолжения вроде «кроме молочной продукции нужно сбалансированное питание»
- [x] Zoya menu mass calculation: Детерминированно рассчитывать 30% массы, полное меню и КБЖУ после получения всех параметров
- [x] Zoya menu integrity: Проверять сумму граммов, сумму КБЖУ и отсутствие продуктов вне confirmed-плана
- [x] Zoya menu state tests: Добавить сквозной tRPC/SSE-тест всего пользовательского диалога без вызова LLM
- [x] Zoya menu state validation: Выполнить полный Vitest, TypeScript и production build перед чекпоинтом
- [x] Zoya rebuild audit: Инвентаризировать текущие nutrition-профили, чат-сессии, продуктовый контекст, RAG и оба runtime-пути
- [x] Zoya profile model: Спроектировать один основной и несколько дополнительных профилей питания на аккаунт
- [x] Zoya profile schema: Добавить пол, дату рождения, рост, вес, цель, активность, аллергии, ограничения, предпочтения и метаданные подтверждения
- [x] Zoya profile API: Реализовать создание, выбор, редактирование, назначение основного профиля и безопасное удаление дополнительных профилей
- [x] Zoya profile questionnaire: Создать анкету первого персонального запроса с обязательными полями и явным подтверждением отсутствия ограничений
- [x] Zoya profile session UX: Показывать компактное подтверждение активного профиля с действиями «Использовать», «Изменить», «Выбрать другой»
- [x] Zoya context assembler: Объединять профиль, подтверждённый продуктовый план, доступные варианты, RAG-знания и недостающие уточнения
- [x] Zoya evidence levels: Разделять факты клиента, факты фермы, проверенные знания и справочные оценки КБЖУ
- [x] Zoya AI orchestration: Формировать структурированный prompt и использовать единый внешний AI-контур для tRPC и SSE
- [x] Zoya response validation: Проверять продукты, ограничения, медицинскую безопасность, арифметику и маркировку справочных данных до показа ответа
- [x] Zoya recommendation UX: Выводить «Учтено в расчёте», основной вариант, 1–2 замены, КБЖУ, долю продукции и источники
- [x] Zoya rebuild tests: Покрыть мультипрофили, анкету, выбор профиля, уточнения, продуктовый RAG, AI и валидацию сквозными тестами
...
- [x] Zoya post-publish verification: Убедиться, что новый запрос меню сначала проходит profile gate и confirmed-product context
- [x] Zoya post-publish verification: Повторить точную формулировку «Зоя мне нужно меню на завтра с моими продуктами» без вымышленных позиций
- [x] Zoya post-publish verification: Проверить tRPC/SSE, health, browser console и network после синхронизации production
